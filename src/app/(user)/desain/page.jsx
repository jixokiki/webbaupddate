...
const ApprovalCuti = () => {
  ...
  useEffect(() => {
    const fetchCutiRequests = async () => {
      ...
      setCutiRequests(requests);
    };
    fetchCutiRequests();
  }, []);

  const handleApprove = async (id, email, duration) => {
    try {
      const userDocRef = doc(db, "usersCuti", email);
      const userDoc = await getDocs(userDocRef);

      if (userDoc.exists()) {
        const userData = userDoc.data();
        const newTotalCuti = userData.totalCuti - duration;

        // Update the user's totalCuti in Firestore
        await updateDoc(userDocRef, {
          totalCuti: newTotalCuti,
        });

        // Update the local state to reflect the approved leave
        setCutiRequests((prevRequests) =>
          prevRequests.map((request) =>
            request.id === id ? { ...request, status: "Approved", totalCuti: newTotalCuti } : request
          )
        );
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="max-w-xl mx-auto p-6 bg-white md:border rounded-md md:shadow-md mt-36">
        <h2 className="text-2xl font-semibold mb-6">Approval Pengajuan Cuti</h2>
        {cutiRequests.map((request) => (
          <div key={request.id} className="mb-4 p-4 border rounded">
            <p>
              <strong>{request.fullname}</strong> ({request.email}) mengajukan cuti selama {request.duration} hari.
            </p>
            <button
              className="mt-2 py-2 px-4 bg-green-500 hover:bg-green-700 text-white font-semibold rounded-md"
              onClick={() => handleApprove(request.id, request.email, request.duration)}
            >
              Approve
            </button>
          </div>
